...
function Slap (opts) {
  ...
  self.actionList = new ActionList({parent: self});
}
Slap.global = null;
...
Slap.prototype._initHandlers = function () {
  ...
  self.declareBindables({
    ...
    'toggleActionList': {
      action: function () {
        if (self.actionList !== self.getCurrentPane()) {
          self.actionList.setCurrent();
        } else {
          self.actionList.close();
        }
        return false;
      }
    },
  ...
  self.on('element keypress', function (el, ch, key) {
    ...
    if (binding && self.hasBindable(binding)) {
      return self.executeBindable(binding, []);
    }
  });
